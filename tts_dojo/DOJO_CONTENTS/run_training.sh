...
# Look up user IDs for $USER to avoid permission issues with folder shared by docker container (tts_dojo)
export TMS_USER_ID=$(id -u)
export TMS_GROUP_ID=$(id -u)
...
#launch docker container from Textymcspeechy directory
cd ../..
...
read
# come back to this dojo
cd tts_dojo/$DOJO_NAME
